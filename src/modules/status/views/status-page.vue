<template>
    <div class="flex justify-center my-16">
      <div v-if="$state.state === 'loading'">
        <svg
          aria-hidden="true"
          class="w-8 h-8 mr-2 text-dark-green animate-spin fill-green"
          viewBox="0 0 100 101"
          fill="none"
      ...
      <table v-else-if="$state.state === 'result'" class="shadow-2xl">
        <thead>
        ...
        <tbody class="divide-y divide-cold-gray">
          ...
          >
            <td class="italic px-6 py-3 text-left">{{ service.name }}</td>
  ...
  const { $state, fetchStatuses } = useStatusStore()
  
  const getStatus = (status: Status): string => (status === 'UP' ? '✅' : '❌')
